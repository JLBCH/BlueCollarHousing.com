-- BlueCollarHousing · 0023_past_due_grace
-- Keep a listing publicly visible during Stripe's payment-retry window
-- (`past_due`) instead of yanking it the instant a recurring charge fails.
-- Stripe retries for ~2 weeks before canceling; the landlord is emailed to
-- update their card and the listing stays up in the meantime.
-- Only the WHERE gate changes vs 0022.
